...
function Threads({
  ...
  useEffect(() => {
    ...
    function resize() {
      ...
      const dpr =
        longestSide > maxRenderDim ? (baseDpr * maxRenderDim) / longestSide : baseDpr;
      renderer.dpr = dpr;
    ...
    function syncMouseTarget() {
      mouseFrameId = 0;
      if (!latestMousePosition) return;
      const rect = container.getBoundingClientRect();
      if (!rect.width || !rect.height) return;

      const [clientX, clientY] = latestMousePosition;
    ...
    function handleMouseMove(event) {
      latestMousePosition = [event.clientX, event.clientY];
      if (!mouseFrameId) {
        mouseFrameId = requestAnimationFrame(syncMouseTarget);
      }
    }
    ...
    function renderFrame(time) {
      const {
        ...
        enableMouseInteraction: nextEnableMouseInteraction,
      } = propsRef.current;
    ...
    function update(time) {
      ...
      hasRenderedStaticFrame = false;
      if (time - lastRenderTime < targetFrameMs) return;
